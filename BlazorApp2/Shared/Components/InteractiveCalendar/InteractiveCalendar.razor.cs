...
public class InteractiveCalendarBase : ComponentBase {
    ...
    [Parameter]
    public UsuarioResponse? usuarioCalendario { get; set; }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender) {
        ...
        if (firstRender) {
            IEnumerable<CalendarioVacacionesResponse> vacaciones;
            if (this.usuarioCalendario != null) {
                vacaciones = await _api.GetUsuarioCalendarioVacacionesAsync(usuarioCalendario.IdTecnico);
            }
            else { 
            ...
            ActualizarDiasCalendario(vacaciones, tiposDias,estados);
        }
    }
    ...
    public void ActualizarDiasCalendario(IEnumerable<CalendarioVacacionesResponse> vacaciones,
                                          IEnumerable<TipoDiaCalendarioResponse> tiposDias, IEnumerable<EstadoCalendarioVacacionesResponse> estados) {
        this.diasCalendario = GenerarDatosCalendario(vacaciones, tiposDias,estados);
        this.datosDiasIniciales = GenerarDatosCalendario(vacaciones, tiposDias, estados);
        StateHasChanged();
    }
    /// <summary>
    ...
    public DatosDias GenerarDatosCalendario(IEnumerable<CalendarioVacacionesResponse> vacaciones, IEnumerable<TipoDiaCalendarioResponse> tiposDias, IEnumerable<EstadoCalendarioVacacionesResponse> estados) {
        ...
        vacaciones.ToList().ForEach(x => {
            ...
            if (dia != null) {
                ...
                var estadoDia = estados.First(X => X.Id == vacaciones.First(X => X.FechaCalendario.Date == dia.Date).Estado).Descripcion;
                dia.ColorSeleccion = tipoDia?.ColorRepresentacion;
                dia.TipoDia = tipoDia?.Descripcion;
                dia.Estado = estadoDia;
            }
        });
    ...
    public void NotifyDayChanges() {
        var ResultadosCambios = this.datosDiasIniciales.Equals(this.diasCalendario);
        NotifyChange.InvokeAsync(ResultadosCambios);
        StateHasChanged();
    }
